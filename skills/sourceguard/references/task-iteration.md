...
The native and regression lanes reuse one frozen input rather than reconstructing it as prose. A new gap records information discovered by the observation; it is not by itself a quality gain. If a provider or external source is missing, preserve that current status and the named gap instead of fabricating a source or observation.
...
If an information blueprint is active, compute revision impact from the exact changed source, anchor, observation, graph edge, target-unit interface, or receipt identity. Reopen only declared consuming gaps, stop decisions, claim uses, handoffs, and parent units. Unknown ownership blocks and suppresses partial impact; it never selects every branch.
