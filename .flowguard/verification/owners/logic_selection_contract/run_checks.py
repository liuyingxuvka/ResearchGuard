...
def _write_native_results() -> None:
    ...
    for index, case_id in enumerate(cases):
        ...
        rows.append({"schema_version": "flowguard.native_model_case_result.v1", "owner_id": f"model:{model.MODEL_ID}", "source_case_id": case_id, "outcome": "pass" if index in (0, 1) else "rejected", "observed_status": "pass" if index in (0, 1) else "blocked", "observed_finding_codes": [] if index in (0, 1) else [model.PROTECTED_FAILURE_IDS[index - 2]], "executed_dimensions": list(dimensions), "oracle_results": [{"dimension": d, "oracle_member_id": f"{model.MODEL_ID}:oracle", "status": "pass" if index == 0 else "blocked", "ok": True} for d in dimensions], "result_artifact_fingerprint": raw_hash, "input_fingerprint": raw_hash, "model_fingerprint": _sha256(MODEL_PATH), "code_fingerprint": _sha256(MODEL_PATH), "test_fingerprint": _sha256(RUNNER_PATH), "oracle_fingerprint": _sha256(RUNNER_PATH), "toolchain_fingerprint": _sha256(RUNNER_PATH), "environment_fingerprint": _sha256(RUNNER_PATH), "raw_artifact_path": raw.name, "child_case_ids": []})
