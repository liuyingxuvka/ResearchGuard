...
MODEL_PATH = ROOT / ".flowguard" / "models" / "researchguard_skillguard_maintenance_model.py"
...
def _review_mesh(model) -> list[str]:
    ...
    if {row["member_id"] for row in structure_children} != members:
        findings.append("StructureMesh does not cover exactly five members")
    if {row["member_id"] for row in test_children} != members:
        findings.append("TestMesh does not cover exactly five members")
    if len(model.DECLARED_CHECKS) != 20 or len(set(model.DECLARED_CHECKS)) != 20:
        findings.append("declared check inventory is not exactly twenty unique ids")
    owned_checks = [check_id for row in test_children for check_id in row["owned_check_ids"]]
    if set(owned_checks) != set(model.DECLARED_CHECKS) or len(owned_checks) != 20:
