...
The model extends the five current member contract exports. It governs only
...
def declared_check_ids(member: str) -> tuple[str, ...]:
    model = build_contract_model(member)
    obligations = tuple(row["obligation_id"] for row in model["obligations"])
    # Each current member declares the same four executable checks.  The
    # umbrella's additional consumer-install obligation is owned by its native
    # tests check; it is not a fifth execution route.
    if len(obligations) not in {4, 5}:
        raise ValueError(f"{member} has an unexpected target-owned obligation inventory")
    return (
...
STRUCTURE_MESH = {
    ...
    "facade": "single researchguard Python distribution plus five consumer SKILL.md entrypoints",
    "children": [
        {
            "member_id": member,
...
TEST_MESH = {
    ...
    "inventory_revision": "five-members-twenty-target-declared-checks",
...
class BindAuthorUnit:
    ...
    output_description = "one five-member maintenance-unit authority or a visible block"
    idempotency = "the same five-member inventory binds the same single unit"
...
class ValidateDeclaredMember:
    ...
    input_description = "one member's exact four target-declared checks"
    output_description = "four current same-unit check identities or a visible block"
    ...
    def apply(self, request: MaintenanceRequest, state: MaintenanceState):
        ...
        if set(state.compiled_members) != MEMBER_SET or request.member_id not in MEMBER_SET:
            ...
            return
        if request.declared_check_count != 4:
            additions = max(0, request.declared_check_count - 4)
...
def maintenance_invariants() -> tuple[Invariant, ...]:
    def one_unit(state: MaintenanceState, _trace):
        if state.maintenance_unit_id and (
            ...
            return InvariantResult.fail("author maintenance escaped the sole five-member unit")
    ...
    return (
        Invariant("one_five_member_unit", "One suite unit owns exactly five member author boundaries.", one_unit),
        ...
        Invariant("exact_twenty_check_inventory", "Validated checks are a subset of the exact twenty target declarations.", exact_checks),
    )
...
def scenarios() -> tuple[Scenario, ...]:
    ...
    return (
        Scenario(
            name="SGM01_current_five_member_closure",
            ...
            expected=ScenarioExpectation(expected_status="ok", required_trace_labels=("author_unit_bound", "compiled_traceguard", "validated_traceguard", "unit_terminal_pass"), summary="five-member maintenance closes without install"),
            workflow=workflow,
        ...
        Scenario(
            ...
            description="A fifth inferred domain check cannot enter the frozen target inventory.",
            initial_state=MaintenanceState(),
            external_input_sequence=(MaintenanceRequest("adopt"), *compiled, MaintenanceRequest("validate", member_id="researchguard", declared_check_count=5)),
